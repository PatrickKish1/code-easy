...
class AIService {
  ...
  async generateCode(request: CodeGenerationRequest): Promise<CodeGenerationResponse> {
    ...
    // Resolve project files from Appwrite if requested and not provided
    let resolvedProjectFiles = context?.projectFiles;
    if ((!resolvedProjectFiles || resolvedProjectFiles.length === 0) && context?.appwriteProjectId) {
      try {
        const { databases, config } = getAppwriteClient();
        const docs = await databases.listDocuments(config.databaseId, config.filesCollectionId, [] as any);
        resolvedProjectFiles = (docs.documents || []).map((d: any) => ({ path: d.path as string, content: d.content as string }));
      } catch (e) {
        console.warn("Failed to fetch Appwrite project files for context:", e);
      }
    }
...
Current context:`;
    ...
    if (context?.appwriteProjectId) {
      prompt += `\n- Remote project: Appwrite project ${context.appwriteProjectId}`;
    }
